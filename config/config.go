...
// SaveAgentProfile persists the selected agent-side profile for a project's agent.
// It uses surgical text editing to preserve comments and unknown fields.
func SaveAgentProfile(projectName, profile string) error {
	configMu.Lock()
	defer configMu.Unlock()
	return patchProjectAgentOption(projectName, "agent", profile)
}
